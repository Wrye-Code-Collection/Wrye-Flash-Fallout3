...
def readRecord(record, melSet=0, skipLabel=0):
    ...
    if hasattr(record, 'getHeader'):
        ...
        attr = 'flags'
        print ' ' * indent + attr.ljust(
            longest) + ' :', record.flags1.hex(), record.flags1.getTrueAttrs()
        attr = 'formID'
        print ' ' * indent + attr.ljust(longest) + ' : ' + bosh.strFid(
            record.fid)
        attr = 'unk'
...
def importRacialEyesHair(srcMod, srcRaceEid, dstMod, dstRaceEid):
    ...
    if dstMod.lower() == 'fallout3.esm':
        raise "You don't REALLY want to overwrite Fallout3.esm, do you?"
    ...
    for record in dstFile.RACE.records:
        if record.eid == dstRaceEid:
            dstRace = record
            break
    if not srcRace: raise "Didn't find race (eid) %s in %s." % (
    srcRaceEid, srcMod)
    if not dstRace: raise "Didn't find race (eid) %s in %s." % (
    dstRaceEid, dstMod)
    # --Get mapper
...
def diffScripts2(oldFile, newFile):
    ...
    for scripts, fileName in ((oldScripts, oldFile), (newScripts, newFile)):
        ...
        scripts.update(dict(
            (record.eid, record.scriptText) for record in modFile.SCPT.records))
    modDump = (GPath(oldFile) + ' to ' + GPath(
        newFile) + ' - Modified Scripts' + '.txt').open('w')
    addDump = (GPath(oldFile) + ' to ' + GPath(
        newFile) + ' - Added Scripts' + '.txt').open('w')
...
def scriptVars(fileName=None, printAll=None):
    ...
    for record in sorted(modFile.SCPT.records, key = lambda a: a.eid):
        ...
        if printAll or (indices != range(1, len(indices) + 1)):
            ...
            record.eid, record.numRefs, record.lastIndex)
            refVars = set(record.references)
            for var in record.vars:
                print ' ', var.index, var.name, ('', '[REF]')[
                    var.index in refVars]
...
def bookExport(fileName=None):
    ...
    if doImport:
        ...
        for line in ins:
            ...
            if maHeader:
                if eid and buffer: imported[eid] = bosh.winNewLines(
                    buffer.getvalue())
...
def perfTest():
    ...
    for testClasses in ['bosh.MreClmt', 'bosh.MreCsty', 'bosh.MreIdle',
        ...
        test = Timer(
            'testClasses = (%s,);loadFactory = bosh.LoadFactory(False,*testClasses);modInfo = bosh.modInfos[GPath("Fallout3.esm")];modFile = bosh.ModFile(modInfo,loadFactory);modFile.load(True)' % testClasses,
    ...
    test = 0.0
    total = 0.0
    for testClasses in ['bosh.MreAchr,bosh.MreCell,bosh.MreWrld',
                        'bosh.MreAcre,bosh.MreCell,bosh.MreWrld',
                        'bosh.MreActi', 'bosh.MreAlch', 'bosh.MreAmmo',
                        'bosh.MreAnio', 'bosh.MreAppa', 'bosh.MreArmo',
                        'bosh.MreBook', 'bosh.MreBsgn',
                        'bosh.MreCell,bosh.MreWrld', 'bosh.MreClas',
                        'bosh.MreClot', 'bosh.MreCont', 'bosh.MreCrea',
                        'bosh.MreDial,bosh.MreInfo', 'bosh.MreDoor',
                        'bosh.MreEfsh', 'bosh.MreEnch', 'bosh.MreEyes',
                        'bosh.MreFact', 'bosh.MreFlor', 'bosh.MreFurn',
                        'bosh.MreGlob', 'bosh.MreGmst', 'bosh.MreGras',
                        'bosh.MreHair', 'bosh.MreIngr', 'bosh.MreKeym',
                        'bosh.MreLigh', 'bosh.MreLscr', 'bosh.MreLvlc',
                        'bosh.MreLvli', 'bosh.MreMgef', 'bosh.MreMisc',
                        'bosh.MreNpc', 'bosh.MrePack', 'bosh.MreQust',
                        'bosh.MreRace',
                        'bosh.MreRefr,bosh.MreCell,bosh.MreWrld',
                        'bosh.MreRoad,bosh.MreCell,bosh.MreWrld',
                        'bosh.MreScpt', 'bosh.MreSgst', 'bosh.MreSkil',
                        'bosh.MreSlgm', 'bosh.MreSoun', 'bosh.MreSpel',
                        'bosh.MreStat', 'bosh.MreTes4', 'bosh.MreTree',
                        'bosh.MreWatr', 'bosh.MreWeap',
                        'bosh.MreWthr']:  # ,'"LAND"', '"PGRD"']:
        test = Timer(
            'testClasses = (%s,);loadFactory = bosh.LoadFactory(False,*testClasses);modInfo = bosh.modInfos[GPath("Fallout3.esm")];modFile = bosh.ModFile(modInfo,loadFactory);modFile.load(True)' % testClasses,
            "import bosh;from bolt import GPath").timeit(1)
        print testClasses, ":", test
...
def csFunctions(fileName="CS Functions.txt"):
    ...
    # --Page writer
    def groupLink(group):
        group = re.sub('FOSE', '[[:Category:Fallout3_Script_Extender|FOSE]]',
            group)
...
def getIds(fileName=None):
    ...
    for group in sorted(group_records):
        # print
        print group
        for fid, eid in sorted(group_records[group],
...
def gmstIds(fileName=None):
    ...
    for eid in ['fPlayerDeathReloadTime', 'iMapMarkerVisibleDistance',
                ...
                'fAutoAimMaxDegreesMiss', 'iHoursToRespawnCell',
                'fEssentialDeathTime', 'fJumpHeightMin',
                'fPlayerPipBoyLightTimer', 'iAINumberActorsComplexScene',
                'iHackingMaxWords', 'fGunShellLifetime',
                'fGunShellCameraDistance', 'fGunDecalCameraDistance',
                'iDebrisMaxCount', 'iHackingDumpRate', 'iHackingInputRate',
                'iHackingOutputRate', 'iHackingFlashOffDuration',
                ...
                'iNumberActorsAllowedToFollowPlayer', 'iRemoveExcessDeadCount',
                'iRemoveExcessDeadTotalActorCount',
                'iRemoveExcessDeadComplexTotalActorCount',
                'iRemoveExcessDeadComplexCount', 'fRemoveExcessDeadTime',
                'fRemoveExcessComplexDeadTime', 'iLevItemLevelDifferenceMax',
                'fMoveWeightMax', ]:
        if eid not in fids:
            maxId += 1
            fids[eid] = maxId
            print '%08X  %08X %s' % (0, maxId, eid)  # --Source file
    if fileName:
        init(3)
    ...
    if maxId > maxOld:
        ...
        print "%d news gmst ids written to Fallout3_ids.pkl" % (
        (maxId - maxOld),)
...
def createTagList(inPath='masterlist.txt', outPath='taglist.txt'):
    ...
    reBashTags = re.compile(
        r'(%\s+{{BASH:|TAG\s+{{BASH:)([^}]+)(}})(.*remove \[)?([^\]]+)?(\])?')
    reDirty = re.compile(
        r'.*?IF\s*\(\s*([a-fA-F0-9]*)\s*\|\s*[\"\'](.*?)[\'\"]\s*\).*?DIRTY:\s*(.*?)\s*$')
...
@mainfunc
def renameArchives(
    root=r'C:\Program Files\Bethesda Softworks\Fallout 3\Downloads'):
    ...
    reTesSource = re.compile(
        r'^\d{4}-(\d[^-]*)-(.+)-TESSource.(zip|rar|7z|ace|exe)$', re.I)
    reTesSourceNV = re.compile(r'^\d{4}-(.+)-TESSource.(zip|rar|7z|ace|exe)$',
        re.I)
    reTesSource3 = re.compile(r'^(.+)-\d+-TESSource.(zip|rar|7z|ace|exe|esp)$',
        re.I)
...
def parseRecords(fileName='Fallout3.esm'):
    ...
    for typed in bush.topTypes:
        ...
        print typed
        if hasattr(getattr(modFile, typed), 'melSet'):
            readRecord(getattr(modFile, typed))
        elif typed == 'CELL':
    ...
    outInfo = bosh.ModInfo(modInfo.dir,
        GPath(modFile.fileInfo.name.s[:-4] + " Dump.esp"))
    ...
    for testClass in testClasses:
        ...
        for record in getattr(modFile, testClass.classType).records:
            ...
            if reBarExt.match(record.model.modPath):
                record.model.modPath = reBarExt.sub(
                    r'Architecture\\BarabusCrypt', record.model.modPath)
...
def balancer(fileName=None):
    ...
    for race in sorted(modFile.RACE.getActiveRecords(),
        ...
        if race.maleStrength != balRace.maleStrength:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[0], balRace.maleStrength - race.maleStrength)
        if race.maleIntelligence != balRace.maleIntelligence:
            print '\t\tset mod%s to %d' % (bush.actorValues[1],
                                           balRace.maleIntelligence - race.maleIntelligence)
        if race.maleWillpower != balRace.maleWillpower:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[2], balRace.maleWillpower - race.maleWillpower)
        if race.maleAgility != balRace.maleAgility:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[3], balRace.maleAgility - race.maleAgility)
        if race.maleSpeed != balRace.maleSpeed:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[4], balRace.maleSpeed - race.maleSpeed)
        if race.maleEndurance != balRace.maleEndurance:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[5], balRace.maleEndurance - race.maleEndurance)
        if race.malePersonality != balRace.malePersonality:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[6], balRace.malePersonality - race.malePersonality)
        if race.maleLuck != balRace.maleLuck:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[7], balRace.maleLuck - race.maleLuck)
        print '\telse'
        if race.femaleStrength != balRace.femaleStrength:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[0], balRace.femaleStrength - race.femaleStrength)
        if race.femaleIntelligence != balRace.femaleIntelligence:
            print '\t\tset mod%s to %d' % (bush.actorValues[1],
            balRace.femaleIntelligence - race.femaleIntelligence)
        if race.femaleWillpower != balRace.femaleWillpower:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[2], balRace.femaleWillpower - race.femaleWillpower)
        if race.femaleAgility != balRace.femaleAgility:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[3], balRace.femaleAgility - race.femaleAgility)
        if race.femaleSpeed != balRace.femaleSpeed:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[4], balRace.femaleSpeed - race.femaleSpeed)
        if race.femaleEndurance != balRace.femaleEndurance:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[5], balRace.femaleEndurance - race.femaleEndurance)
        if race.femalePersonality != balRace.femalePersonality:
            print '\t\tset mod%s to %d' % (bush.actorValues[6],
            balRace.femalePersonality - race.femalePersonality)
        if race.femaleLuck != balRace.femaleLuck:
            print '\t\tset mod%s to %d' % (
            bush.actorValues[7], balRace.femaleLuck - race.femaleLuck)
        print '\tendif'
        ...
        if balRace.skill1 != 255: balBoosts[
            balRace.skill1 - 12] = balRace.skill1Boost
        if balRace.skill2 != 255: balBoosts[
            balRace.skill2 - 12] = balRace.skill2Boost
        if balRace.skill3 != 255: balBoosts[
            balRace.skill3 - 12] = balRace.skill3Boost
        if balRace.skill4 != 255: balBoosts[
            balRace.skill4 - 12] = balRace.skill4Boost
        if balRace.skill5 != 255: balBoosts[
            balRace.skill5 - 12] = balRace.skill5Boost
        if balRace.skill6 != 255: balBoosts[
            balRace.skill6 - 12] = balRace.skill6Boost
        if balRace.skill7 != 255: balBoosts[
            balRace.skill7 - 12] = balRace.skill7Boost

        # --Attributes
        for index, boost, balBoost in zip(range(21), boosts, balBoosts):
            if boost != balBoost:
                print '\tset mod%s to %d' % (
                skillNames[index], balBoost - boost)
