...
# ===================================================
# Specific parser for Wrye Flash
# ===================================================
...
# InstallerWizard ----------------------------------
#  Class used by Wrye Flash, creates a wx Wizard that
...
class InstallerWizard(wiz.Wizard):
    def __init__(self, link, subs, pageSize, pos):
        ...
        self.parser = WryeParser(self, installer, subs, bArchive, path,
            link.bAuto)
...
class PageSelect(PageInstaller):
    def __init__(self, parent, bMany, title, desc, listItems, listDescs,
        listImages, defaultMap):
        PageInstaller.__init__(self, parent)
...
def generateTweakLines(wizardEdits, target):
    lines = []
    lines.append('; Generated by Wrye Flash NV %s for \'%s\' via wizard' % (
    basher.settings['bash.readme'][1], target.s))
    for realSection in wizardEdits:
        if realSection == ']set[':
            format = wizardEdits[realSection][
                         0] + ' %(setting)s to %(value)s%(comment)s'
        elif realSection == ']setgs[':
            format = wizardEdits[realSection][
                         0] + ' %(setting)s %(value)s%(comment)s'
        else:
            lines.append('')
...
class PageFinish(PageInstaller):
    def __init__(self, parent, subsList, espmsList, espmRenames, bAuto, notes,
        ...
        textTitle = wx.StaticText(self, -1, _(
        ...
        self.checkApply = wx.CheckBox(self, wx.ID_ANY,
            _('Apply these selections'))
        ...
        self.checkInstall = wx.CheckBox(self, wx.ID_ANY,
            _('Install this package'))
        self.checkInstall.SetValue(
            basher.settings['bash.installers.autoWizard'])
...
class PageVersions(PageInstaller):
    def __init__(self, parent, bFoOk, foHave, foNeed, bFOSEOk, foseHave,
        foseNeed, bOBGEOk, obgeHave, obgeNeed, bWBOk, wbHave, wbNeed):
        PageInstaller.__init__(self, parent)
        ...
        self.textWarning = wx.StaticText(self, 124, _(
        ...
        sizerVersionsTop = wx.StaticBoxSizer(
            wx.StaticBox(self, -1, _('Version Requirements')))
        ...
        linkFo = wx.HyperlinkCtrl(self, -1, 'Fallout',
        ...
        linkFOSE = wx.HyperlinkCtrl(self, -1, 'Fallout 3 Script Extender',
        ...
        linkWB = wx.HyperlinkCtrl(self, -1, 'Wrye Flash',
            'http://www.nexusmods.com/fallout3/mods/11336')
        linkWB.SetVisitedColour(linkWB.GetNormalColour())
        linkWB.SetToolTip(wx.ToolTip('http://www.nexusmods.com/fallout3'))
...
class WryeParser(ScriptParser.Parser):
    ...
    def __init__(self, parent, installer, subs, bArchive, path, bAuto,
        ...
        self.SetOperator('+=', self.AssAdd, ScriptParser.OP.ASS,
            ScriptParser.RIGHT)
        self.SetOperator('-=', self.AssMin, ScriptParser.OP.ASS,
            ScriptParser.RIGHT)
        self.SetOperator('*=', self.AssMul, ScriptParser.OP.ASS,
            ScriptParser.RIGHT)
        self.SetOperator('/=', self.AssDiv, ScriptParser.OP.ASS,
            ScriptParser.RIGHT)
        # self.SetOperator('%=', self.AssMod, ScriptParser.OP.ASS, ScriptParser.RIGHT)
        self.SetOperator('^=', self.AssExp, ScriptParser.OP.ASS,
            ScriptParser.RIGHT)
        # Comparison
        ...
        self.SetOperator('!', self.opNot, ScriptParser.OP.NOT,
            ScriptParser.RIGHT)
        self.SetOperator('not', self.opNot, ScriptParser.OP.NOT,
            ScriptParser.RIGHT)
        ...
        self.SetOperator('^', self.opExp, ScriptParser.OP.EXP,
            ScriptParser.RIGHT)

        # --Functions
        self.SetFunction('CompareFalloutVersion', self.fnCompareFalloutVersion,
            1)
        ...
        self.SetKeyword('SelectOne', self.kwdSelectOne, 7,
            ScriptParser.KEY.NO_MAX)
        self.SetKeyword('SelectMany', self.kwdSelectMany, 4,
            ScriptParser.KEY.NO_MAX)
    ...
    def Continue(self):
        ...
        while self.cLine < len(self.lines):
            ...
            try:
                ...
                return PageError(self.parent, _('Installer Wizard'), _(
        ...
        return PageFinish(self.parent, self.sublist, self.espmlist,
            self.espmrenames, self.bAuto, self.notes, self.iniedits)
    ...
    def Ass(self, l, r):
        if l.type not in [ScriptParser.VARIABLE, ScriptParser.NAME]:
            error('Cannot assign a value to %s, type is %s.' % (
            l.text, ScriptParser.Types[l.type]))
        self.variables[l.text] = r.data
        return r.data

    def AssAdd(self, l, r):
        return self.Ass(l, l + r)
    ...
    def opInc(self, l):
        if l.type not in [ScriptParser.VARIABLE, ScriptParser.NAME]:
            error('Cannot increment %s, type is %s.' % (
            l.text, ScriptParser.Types[l.type]))
    ...
    def opDec(self, l):
        if l.type not in [ScriptParser.VARIABLE, ScriptParser.NAME]:
            error('Cannot decrement %s, type is %s.' % (
            l.text, ScriptParser.Types[l.type]))
        self.variables[l.text] = l.data - 1
        return l.data
    ...
    def fnCompareFalloutVersion(self, foWant):
        ret = self._TestVersion(self._TestVersion_Want(foWant),
            bosh.dirs['app'].join('fallout3.exe'))
        return ret[0]

    def fnCompareFOSEVersion(self, foseWant):
        fose = 'fose_loader.exe'
        ret = self._TestVersion(self._TestVersion_Want(foseWant),
            bosh.dirs['app'].join(fose))
        return ret[0]
    ...
    def fnEditINI(self, iniName, section, setting, value, comment=None):
        ...
        self.iniedits[iniPath][realSection][1][realSetting] = (
    ...
    def _KeywordSelect(self, bMany, name, *args):
        ...
        if self.LenFlow() > 0 and self.PeekFlow().type == 'Select' and not self.PeekFlow().active:
            ...
            self.PushFlow('Select', False,
                ['SelectOne', 'SelectMany', 'EndSelect'])
            return
        ...
        self.page = PageSelect(self.parent, bMany, _('Installer Wizard'),
            main_desc, titles, descs, image_paths, defaultMap)

    def kwdCase(self, value):
        if self.LenFlow() == 0 or self.PeekFlow().type != 'Select':
            error(UNEXPECTED % 'Case')
        if value in self.PeekFlow().values or str(
            value) in self.PeekFlow().values:
    ...
    def _SelectSubPackage(self, bSelect, subpackage):
        ...
        if package:
            ...
            error(_(
                "Sub-package '%s' is not a part of the installer.") % subpackage)

    def kwdSelectAll(self):
        self._SelectAll(True)

    def kwdDeSelectAll(self):
    ...
    def kwdSelectEspm(self, espm):
        self._SelectEspm(True, espm)

    def kwdDeSelectEspm(self, espm):
        self._SelectEspm(False, espm)
    ...
    def kwdSelectAllEspms(self):
        self._SelectAllEspms(True)

    def kwdDeSelectAllEspms(self):
        self._SelectAllEspms(False)
        ...
        if espm:
            ...
            if espm.lower()[-4:] != newName.lower()[-4:]:
                raise ScriptParser.ParserError(
                    _('Cannot rename %s to %s: the extensions must match.') % (
                    espm, newName))
    ...
    def kwdRequireVersions(self, fo, fose='None', obge='None', wbWant='0'):
        ...
        if not bFoOk or not bFOSEOk or not bOBGEOk or not bWBOk:
            self.page = PageVersions(self.parent, bFoOk, foHave, ob, bFOSEOk,
                foseHave, fose, bOBGEOk, obgeHave, obge, bWBOk, wbHave, wbWant)

    def _TestVersion_OBGE(self, want):
        retOBGEOld = self._TestVersion(want,
            bosh.dirs['mods'].join('fose', 'plugins', 'obge.dll'))
        retOBGENew = self._TestVersion(want,
            bosh.dirs['mods'].join('fose', 'plugins', 'obgev2.dll'))
    ...
    def _TestVersion(self, need, file):
        if file.exists():
            ...
            have = win32api.HIWORD(ms), win32api.LOWORD(ms), win32api.HIWORD(
    ...
    def kwdReturn(self):
        self.page = PageFinish(self.parent, self.sublist, self.espmlist,
            self.espmrenames, self.bAuto, self.notes, self.iniedits)

    def kwdCancel(self, msg=_("No reason given")):
        self.page = PageError(self.parent,
            _('The installer wizard was canceled:'), msg)
