...
from bolt import _, BoltError, AbstractError, StateError, GPath, Progress, \
    deprint, bUseUnicode
from balt import askSave, askYes, askOpen, askWarning, showError, showWarning, \
    showInfo
...
class BackupSettings(BaseBackupSettings):
    def __init__(self, parent=None, path=None, quit=False, backup_images=None):
        ...
        for path, name, tmpdir in (
            ...
            'Fallout 3 Mods\\Bash Mod Data\\INI Data'), (
            dirs['installers'].join('Bash'), 'Converters',
            'Fallout 3 Mods\\Bash Installers\\Bash'), (
    ...
    def Apply(self):
        ...
        cPickle.dump(self.verDat, out,
            -1)  # data version, if this doesn't match the installed data version, do not allow restore
        cPickle.dump(self.verApp, out,
            -1)  # app version, if this doesn't match the installer app version, warn the user on restore
        out.close()
    ...
    def PromptFile(self):
        ...
        if self.archive == None or self.dir.join(self.archive).exists():
            ...
            if not self.quit:
                path = askSave(self.parent, _('Backup Bash Settings'), self.dir,
                    file, '*.7z')
...
class RestoreSettings(BaseBackupSettings):
    ...
    def Apply(self):
        ...
        if 14 >= self.verApp:
            ...
            restore_paths += ((dirs['l10n'], 'Fallout 3\\Data'),)
            if self.restore_images:
                restore_paths += ((dirs['images'], 'Fallout 3\\Mopy\\images'),)
        else:
            restore_paths += ((dirs['l10n'], 'Fallout 3\\bash\\l10n'),)
            if self.restore_images:
                restore_paths += (
        ...
        for fpath, tpath in restore_paths:
            ...
            if path.exists():
                for name in path.list():
                    if path.join(name).isfile():
                        deprint(
                            GPath(tpath).join(name).s + ' --> ' + fpath.join(
                                name).s)
        ...
        if path.exists():
            for root, folders, files in path.walk(True, None, True):
                ...
                for name in files:
                    deprint(
                        tpath.join(root, name).s + ' --> ' + fpath.join(root,
                            name).s)
