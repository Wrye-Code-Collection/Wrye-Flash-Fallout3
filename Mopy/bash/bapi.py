   ...
   """
    ...
    if not IsCompatibleVersion(*PythonAPIVersion):
        try:
            ...
            ver = ''
        raise BossVersionError(
            'bapi.py is not compatible with the specified BOSS API DLL (%s).' % ver)
    ...
    BOSS_API_GAME_FALLOUTNV = c_uint.in_dll(BAPI,
    ...
    # =========================================================================
    BOSS_API_LOMETHOD_TIMESTAMP = c_uint.in_dll(BAPI,
        'BOSS_API_LOMETHOD_TIMESTAMP').value
    BOSS_API_LOMETHOD_TEXTFILE = c_uint.in_dll(BAPI,
        'BOSS_API_LOMETHOD_TEXTFILE').value
    ...
    def BossErrorCheck(result):
        ...
        if result in {BOSS_API_OK, BOSS_API_OK_NO_UPDATE_NECESSARY}:
            return result
    ...
    # =========================================================================
    class BossDb(object):
        def __init__(self, gamePath, game='Fallout 3'):
        ...
        @property
        def LoadOrderMethod(self):
            return self._LOMethod
        ...
        def Load(self, masterlist, userlist=None):
            ...
            _CLoad(self._DB, _enc(masterlist),
                _enc(userlist) if userlist else None)
        ...
        class LoadOrderList(list):
            ...
            ## LoadOrder[i] = x
            def __setitem__(self, key, value): raise Exception(
                'BossDb.LoadOrder does not support item setting')

            ## del LoadOrder[i]
            def __delitem__(self, key): raise Exception(
                'BossDb.LoadOrder does not support item deletion')

            ## LoadOrder += [s,3,5]
            ##  and other compound assignment operators
            def __iadd__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __isub__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __imul__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __idiv__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __itruediv__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __ifloordiv__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __imod__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __ipow__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __ilshift__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __irshift__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __iand__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __ixor__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            def __ior__(self, other): raise Exception(
                'BossDb.LoadOrder does not support compound assignment')

            ## LoadOrder.append('ahalal')
            def append(self, item): raise Exception(
                'BossDb.LoadOrder does not support append.')

            ## LoadOrder.extend(['kkjjhk','kjhaskjd'])
            def extend(self, items): raise Exception(
                'BossDb.LoadOrder does not support extend.')

            def remove(self, item): raise Exception(
                'BossDb.LoadOrder does not support remove.')

            def pop(self, item): raise Exception(
                'BossDb.LoadOrder does not support pop.')

            ## TODO: Possibly make this call BOSS's auto-sorting
            def sort(self, *args, **kwdargs): raise Exception(
                'BossDb.LoadOrder does not support sort.')

            def reverse(self, *args, **kwdargs): raise Exception(
                'BossDb.LoadOrder does not support reverse.')
        ...
        class ActivePluginsList(list):
            ...
            def __setitem__(self, key, value):
                raise Exception(
                    'BossDb.ActivePlugins does not support item setting')

            ## LoadOrder *= 3
            ##  and other compound assignment operators
            def __imul__(self, other):
                raise Exception(
                    'BossDb.ActivePlugins does not support compound assignment')
            ...
            def pop(self, item):
                raise Exception('BossDb.ActivePlugins does not support pop.')

            def sort(self, *args, **kwdargs):
                raise Exception('BossDb.ActivePlugins does not support sort.')

            def reverse(self, *args, **kwdargs):
                raise Exception(
                    'BossDb.ActivePlugins does not support reverse.')
        ...
        def GetDirtyMessage(self, plugin):
            message = c_uint8_p()
            clean = c_uint32()
            _CGetDirtyMessage(self._DB, _enc(plugin), byref(message),
                byref(clean))
